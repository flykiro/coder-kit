...
function getHSLAString(colors: number[], alpha = 1, withAlpha = false) {
  const [hue, saturation, lightness] = convertRGBToHSL(colors)

  return `hsl${withAlpha ? 'a' : ''}( ${hue}, ${saturation}%, ${lightness}%${withAlpha ? `, ${alpha}` : ''} )`
}

function convertColor (conversionType: CssColorTypes, colors: number[], alpha: number = 1) {
...
function convertRGBToHSL(rgb: number[]) {
...
class CSSColorConverter extends PureComponent<IProps, IState> {
  ...
  onTapCopy = () => {
    const result = copy(this.state.output)
